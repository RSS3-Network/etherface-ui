...
const Hash = () => {
  ...
  const columnsSignature = [
    ...
    },
    {
      ...
      accessor: "hash",
      style:
        "underline underline-offset-4 duration-200 cursor-pointer hover:text-black",
      clickAction: (event) => {
        setInput(event);
        setQuery(event);
      },
  ...
  const fetcherHashEndpoint = async (query: string, page: number) => {
    ...
    let response = await axios.get(
      `/v1/signatures/hash/${queryKind}/${query}/${page}`,
      {
    ...
    <Layout>
      ...
      <div className="grid grid-cols-8">
          ...
          <div className="col-start-2 col-end-8 space-y-2 mt-4">
            <div className="space-y-2">
              <SearchBar
                input={input}
                placeholder={
                  "Find signatures by their hash, e.g. 0x70a08231 or the full hash"
                }
                ...
                <Alert
                  ...
                  infoMessage={
                    <div className="flex flex-col text-left">
                      <span>Some tips:</span>
                      <span>
                        - Searches are case sensitive yielding signatures
                        starting with your query
                      </span>
                      <span>
                        - Filtering by function, event and error signatures is
                        supported by starting your query with
                      </span>
                      <span className="ml-4">
                        - <code>f#</code> to find functions only, e.g.{" "}
                        <code>f#0x70a08231</code>
                      </span>
                      <span className="ml-4">
                        - <code>e#</code> to find events only, e.g.{" "}
                        <code>e#0x2d339b1e</code>
                      </span>
                      <span className="ml-4">
                        - <code>err#</code> to find errors only, e.g.{" "}
                        <code>err#0xfe7507e6</code>
                      </span>
                    </div>
                  }
            ...
            {signature && (
              <div className="text-center font-mono">{signature.text}</div>
            )}
          </div>
          ...
          <div className="mt-2">
            {hashCollision && (
              <Alert
                kind="info"
                infoMessage={
                  "Found multiple signatures, to see their sources pick one by clicking on their hash"
                }
                errorMessage={undefined}
                statusCode={null}
                query={null}
        ...
        <div className="col-start-2 col-end-8">
          <div className="flex justify-end">
            {signature && (
              <button
                ...
                onClick={() => {
                  showSources === "Github"
                    ? setShowSources("Etherscan")
                    : setShowSources("Github");
                }}
              >
                {showSources === "Github"
                  ? "Show Etherscan sources"
                  : "Show Github sources"}
              </button>
            )}
          </div>

          {!signature && query.length != 0 && (
            <Table
          ...
          {signature && showSources === "Etherscan" && (
            <Table
